...
const ai = new GoogleGenAI({
  ...
  apiKey: "AIzaSyDWPzABaKZNXBGH6b103XfWXGxnZe7Pmbc", // mohit bunty
  ...
  try {
    ...
    for (const reason of finishReasons) {
      switch (reason) {
        ...
        case "SAFETY":
          console.log(
            "🚫 Generation blocked due to safety filters (text safety)."
          );
          toast.error(
            " Generation blocked due to safety filters (text safety)."
          );
        ...
        case "IMAGE_SAFETY":
          console.log(
            "🚫 Image generation blocked due to image safety filters."
          );
          toast.error(" Image generation blocked due to image safety filters.");
          break;
        case "OTHER":
          console.log(
            "❓ Generation stopped for an unspecified reason (OTHER)."
          );
